...
# Nick Modifications ------------------------------------------------------
#Nick made some modifications to plotting code and added points with labels zoomed in on the WEBCA area

tt<-read.csv("data/merged_edna_samples.csv",header = T)
...
p1 <- ggplot()+
  ...
  geom_point(data=tt,aes(x = longitude,y=latitude))+
  ggrepel::geom_label_repel(data=tt,aes(x=longitude,y=latitude,label = edna_sample_name))+
  coord_sf(xlim=c(-60,-64),ylim=c(42,44.5),expand = F)+
