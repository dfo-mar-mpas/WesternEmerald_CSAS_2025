...
edna_df <- read.csv("data/merged_edna_samples.csv")%>% #this is the eDNA sample metadata from the 2020 survey where it was collected 
           st_as_sf(coords=c("longitude","latitude"),crs=latlong)%>%
           st_join(.,edna_buffer,join=st_intersects)%>%
           filter(!is.na(name))%>%
           dplyr::select(-name)%>%
           st_join(., maritimes_network%>%rbind(webca_buffer), join = st_intersects)%>%
           mutate(name = ifelse(is.na(name),"buffer_50km",name),
                  stations=gsub("-","_",edna_sample_name))%>%
           distinct(stations,.keep_all=T)

edna_species_df <- read.csv("data/2020eDNA_COI12S16S_FinalTaxaTable.csv")%>%
                  dplyr::select(c("OTU_ID",edna_df$stations))%>%
                  rowwise() %>%
...
edna_species <- edna_species_df%>%pull(latin)%>%unique()

tax_list_edna <- data.frame()
...
for(i in 1:length(edna_species)){
  ...
  tax_list_edna <- rbind(tax_list_edna,temp3)
  
}                     

edna_formatted <- tax_list_edna%>%
  dplyr::select(all_of(c(PhyloNames,"aphiaID","species_filter")))%>%
  rename(latin = species_filter)

edna_data <- edna_species_df%>%
             left_join(.,edna_formatted)%>%
